  ...
  xhttp.onreadystatechange = function() {
      if (xhttp.readyState == 4 && xhttp.status == 200) {
        ...
        for (i = 0; i < notices.length; i ++) {
        	var noticeID = notices[i].url.substring(54);
            noticeList.append("<li><a href='http://www.ridewta.com/Pages/notice-details.aspx?ID=" + noticeID + "'>" + notices[i].title + "</a></li>");
        }
        setTimeout(loadNotices, 300000);
      }
    }
  xhttp.open("GET", "https://pi1s1ct7mb.execute-api.us-west-2.amazonaws.com/prod/notices/en/", true);
  xhttp.send();
}
