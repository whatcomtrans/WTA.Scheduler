...
var selectedRoute, trip_headsign;
...
function flipRoute() {
	...
	$('#busTable').empty();
	$('.routeNumber').empty();
	displaySelectedRoute(currentRouteID, currentServiceID, currentDirectionID);
	if (trip_headsign === -1) {
...
function displaySelectedRoute(routeID, serviceID, directionID) {
	//define options:
	...
	for (i=0;i<trips.length;i++) {
	    if (trips[i].route_id == routeID && trips[i].direction_id == directionID && trips[i].service_id == serviceID) {
